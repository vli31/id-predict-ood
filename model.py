...
class MLPClassifier(nn.Module):
    ...
    def __init__(self, input_dim:int, hidden_dim:int, label_size:int, num_layers:int, dropout_rate:float):
        ...
        # Output layer
        self.output_layer = nn.Linear(hidden_dim, label_size)
        self.dropout = nn.Dropout(dropout_rate)
...
def get_MLP(hidden_dim:int, num_layers:int=2, dropout_rate:float=0.0, 
            input_dim:int=MAX_LEN+2, label_size:int=N_CLASSES) -> MLPClassifier:
    
    model = MLPClassifier(input_dim, hidden_dim, label_size, num_layers, dropout_rate)
